...
//        let result = try await connection.query(query).get()
//        let rows = result.rows
//        
//        return try rows.map { row in
//            // Access columns directly and convert to the appropriate type
//            guard let id = row.column("id")?.string,
//                  let title = row.column("title")?.string,
//                  let isCompleted = row.column("isCompleted")?.bool else {
...
//        guard let connection else { throw DatabaseError.connectionNotAvailable }
//        
